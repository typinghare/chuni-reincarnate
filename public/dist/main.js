...
/***/ "./node_modules/link-oriented/src/index.js":
/*!*************************************************!*\
  !*** ./node_modules/link-oriented/src/index.js ***!
  \*************************************************/
...
exports.Links = exports.$var = exports.$null = exports.Link = exports.Var = void 0;
class Var {
    /**
     * Constructor.
    ...
    /**
     * <val> getter.
     * @example const a = new Var('demo'); console.log(a.val);  // demo
     */
    get val() {
        return this._val;
    }
    /**
     * <val> setter. This is used to assign value for variable without triggering.
     * @param val
     */
    set val(val) {
        this._val = val;
    ...
    /**
     * When using template strings (ES6 or newer), variable can be printed as string of its value.
     * @example const a = new Var('demo'); console.log(`${a}`); // demo
     */
    toString() {
     ...
     */
    trigger() {
        manager.invokeAll(this);
    }
    /**
     * Release self. All related triggers are to bind.
     */
    release() {
        manager.releaseVar(this);
    }
    /**
     * Create a link that use self as head variable.
     * @param tail tail of the link
     * @param trigger trigger function
     */
    link(tail, trigger = null) {
        const link = new Link(this, tail);
        if (trigger)
...
class Link {
    ...
    set trigger(func) {
        this._trigger = func;
        manager.bind(this._head, this._tail, this._trigger);
    }
    /**
     * Turn off trigger.
     */
    off() {
        manager.unbind(this._head, this._tail);
    }
    /**
     * Turn on trigger.
     */
...
exports.Link = Link;
/**
 * Manager to manage links and triggers.
 */
class Manager {
    constructor() {
        /**
         * a map storing trigger functions.
         * form: Map<head, <tail, trigger>>
         * @private
         */
        this._forwardMap = new Map();
        /**
         * a map storing trigger functions.
         * form: Map<tail, <head, trigger>>
         * @private
         */
        this._backwardMap = new Map();
    }
    /**
     * Retrieve a map from forward map.
     * @param head head variable
     * @param create whether to create a new map if not found.
     * @private
     */
    _retrieveForward(head, create = false) {
        let map = this._forwardMap.get(head);
        if (create && map === undefined) {
            map = new Map();
            this._forwardMap.set(head, map);
        }
        return map || null;
    }
    /**
     * Retrieve a set from backward map.
     * @param tail tail variable
     * @param create whether to create a new set if not found.
     * @private
     */
    _retrieveBackward(tail, create = false) {
        let set = this._backwardMap.get(tail);
        if (create && set === undefined) {
            set = new Set();
            this._backwardMap.set(tail, set);
        }
        return set || null;
    }
    /**
     ...
     * @param trigger trigger function
     */
    bind(head, tail, trigger) {
        // forward
        const forwardMap = this._retrieveForward(head, true);
        if (forwardMap instanceof Map) {
            forwardMap.set(tail, trigger);
        }
        // backward
        this._retrieveBackward(tail, true).add(head);
    }
    /**
     * Unbind a trigger.
     * @param head
    ...
    unbind(head, tail) {
        // forward
        const map = this._retrieveForward(head);
        if (map instanceof Map) {
            map.delete(tail);
            if (map.size == 0)
                this._forwardMap.delete(head);
        }
        // backward
        const set = this._retrieveBackward(tail);
        if (set instanceof Set) {
            set.delete(head);
            if (set.size == 0)
                this._backwardMap.delete(head);
        }
    }
    /**
     * Release a variable, unbind all related triggers.
     * @param _var
     */
    releaseVar(_var) {
        // as head
        const map = this._retrieveForward(_var);
        if (map !== null) {
            map.forEach((_, tail) => this._retrieveBackward(tail).delete(_var));
        }
        this._forwardMap.delete(_var);
        // as tail
        let set = this._retrieveBackward(_var);
        if (set !== null) {
            set.forEach((_, head) => this._retrieveForward(head).delete(_var));
        }
        this._backwardMap.delete(_var);
    }
    /**
     * Invoke all trigger functions of specified head variable.
     * @param head
     */
    invokeAll(head) {
        const map = this._retrieveForward(head);
        const cb = (trigger, tail) => {
 ...
 * A variable that
 */
exports.$null = new Var(null);
/**
 * A syntactic sugar, [$var(val)] is equivalent to [new Var(val)].
 * form: head -> tail.
 * @param value value of variable
 */
const $var = function (value = null) {
...
(function (Links) {
     ...
     * @param head head variable
     * @param tail tail variable
     * @param forwardFunction forward trigger function
     * @param backwardFunction backward trigger function
     */
    function twoWayBinding(head, tail, forwardFunction, backwardFunction) {
        const forwardLink = head.link(tail), backwardLink = tail.link(head);
        forwardLink.trigger = () => {
            backwardLink.off();
            tail.assign(forwardFunction(head, tail));
            backwardLink.on();
            return;
        };
        backwardLink.trigger = () => {
            forwardLink.off();
            head.assign(backwardFunction(tail, head));
            forwardLink.on();
            return;
        };
    }
    Links.twoWayBinding = twoWayBinding;
    /**
     * Trigger when specified variable is assigned
     * @param _var a variable
     * @param trigger trigger function
     */
    function trigger(_var, trigger) {
        _var.link(exports.$null, (v) => trigger(v));
    }
    Links.trigger = trigger;
    /**
     * Log on console when specified variable is assigned.
     * @param _var a variable
     * @param caption
     */
    function log(_var, caption = '') {
        trigger(_var, () => {
            caption ? console.log(`${caption}: ${_var}`) : console.log(_var.val);
        });
    }
    Links.log = log;
...
const interface_1 = __webpack_require__(/*! ./interface */ "./src/interface.js");
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const TYPES = {
    DataContainer: Symbol.for('DataContainer'),
